...
Outputs (full mode):
  - ncei/derived/singlebeam/points_raw/<track_id>.parquet  (one per track)
...
def standardize_one_track(
    ...
    if has_time and time_full is not None:
        ...
        if not pd.notna(pd.Series(time_arr)).any():
            has_time = False
            time_arr = np.array([pd.NaT] * n_points_out, dtype="datetime64[ns]")
    else:
        time_arr = np.array([pd.NaT] * n_points_out, dtype="datetime64[ns]")
...
def process_one(
    ...
    out_path = DERIVED_DIR / f"{track_id}.parquet"
